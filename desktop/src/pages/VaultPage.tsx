...
import { apiGenerateAITask, apiVerifyAITask, AITaskResponse } from "../api/prodoApi";
...
const VaultPage: React.FC = () => {
  ...
  const handleRequestWaiver = async (type: "math" | "essay") => {
    setWaiverMsg(null);
    setWaiverAnswer("");
    try {
      const res = await apiGenerateAITask(type);
      if (res.success) {
    ...
    try {
      ...
      if (res.success) {
        ...
        setTimeout(() => {
          ...
          // Reload page to re-sync XP balance from SQLite db
